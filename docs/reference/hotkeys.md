...
| `Ctrl+Alt+Shift+P` | Open the [command palette](/features/command-palette) |
| `Ctrl+Shift+T` | Toggle [Smart Terminal](/features/smart-terminal) |

`Ctrl+Alt+Shift+P` also restores the toolbar if it was hidden.
...
Cursor maps `Ctrl+Shift+P` to its own command palette. VibeBar uses `Ctrl+Alt+Shift+P` by default to avoid the conflict. If the palette does not open, focus the toolbar first or disable VibeBar hotkeys in Settings.
