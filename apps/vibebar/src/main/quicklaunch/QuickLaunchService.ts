...
import type { QuickLaunchApp, QuickLaunchResult, VibeSettings } from '@shared/types.js'
...
export class QuickLaunchService {
  ...
  async launch(
    ...
    try {
      ...
      const pasteDecision = await this.resolvePasteDecision(this.store.getSettings())
      if (pasteDecision === 'cancel') {
        return { ok: true, pasteNotice: 'Launch cancelled at pre-paste safety gate.' }
      }
      if (pasteDecision === 'copy-only') {
        return {
          ok: true,
          pasteAttempted: false,
          pasteNotice: 'Opened Cursor — paste manually from clipboard.'
        }
      }
  ...
  /** Resolves whether to paste, open without paste, or cancel (pre-paste safety gate). */
  async resolvePasteDecision(settings: VibeSettings): Promise<PrePasteDecision> {
    if (!isPrePasteGateEnabled(settings)) return 'paste'
    let text = ''
    try {
    ...
    return confirmPrePasteGate(scan)
  }
